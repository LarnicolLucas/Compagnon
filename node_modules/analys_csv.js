...
const analys= (text) => {
    ...
    const addGestToArrayFunction = (arrayFunction, item) => {
        ...
        for(let i0 = 0; i0 <item.length -1; i0++){
            for(let i = 0; i<item[i0] - 1; i++){
                //resultats apparaissse 2 fois et parfois ne rend pas d'arary... Manque le nom et l'ID
                res.push((geste, arr, id)=> geste == '' ? arr : arr.push({id: id, nom: "en cours de dev", id_item: i0, id_geste: i, maitrise: Number(geste)}) );
            }
        }
    ...
    if(recursiveCheckNb(0, matrix_row_col, nb_de_ligne, true)){
            ...
            return el.reduce((acc, cur, i) => {
                ...
                if(arrayFunctions[i] == undefined){
                    ...
                    if(i === 1){
                        ...
                        let geste_metier = arrayFunctions[i](cur, list_geste, i);
                        return Object.assign(acc, {geste_metier : geste_metier})
                    } else {
        ...
        return list_intervention_object.filter(el=> typeof el.geste_metier == "object")
    }
}
